...
parent.appendChild(node);node.loader=node.getOwnerTree().loader;if(_node.children){categoryLoader.buildCategoryTree(node,_node.children);}};categoryLoader.buildCategoryTree=function(parent,config){var j=0;if(!config){return null;}
if(parent&&config&&config.length){for(j=0;j<config.length;j++){categoryLoader.processCategoryTree(parent,config,j);}}};categoryLoader.buildHashChildren=function(hash,node){var j=0;if((node.childNodes.length>0)||(node.loaded===false&&node.loading===false)){hash.children=[];for(j=0,len=node.childNodes.length;j<len;j++){if(!hash.children){hash.children=[];}
